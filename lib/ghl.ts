...
function requiredEnv(name: string) {
  ...
  if (!value) {
    throw new Error(`${name} is not set. Add the GHL custom field ID to .env.local.`);
  }
  return value;
}
...
export async function upsertGhlContact(lead: LeadInput) {
  ...
  const serviceFieldId = requiredEnv("GHL_CUSTOM_FIELD_SERVICE_ID");
  const projectDetailsFieldId = requiredEnv("GHL_CUSTOM_FIELD_PROJECT_DETAILS_ID");
  ...
  return ghlFetch("/contacts/upsert", {
    ...
    body: JSON.stringify({
      ...
      customFields: [
        { id: serviceFieldId, value: lead.service },
        { id: projectDetailsFieldId, value: lead.details ?? "" },
      ],
    }),
  });
}
...
/** Log submission metadata that doesn't have a dedicated contact field. */
